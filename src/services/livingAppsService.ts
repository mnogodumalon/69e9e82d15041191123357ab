...
import type { FionaExportZuKml, CreateFionaExportZuKml } from '@/types/app';
...
export class LivingAppsService {
  // --- FIONA_EXPORT_ZU_KML ---
  static async getFionaExportZuKml(): Promise<FionaExportZuKml[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FIONA_EXPORT_ZU_KML}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as FionaExportZuKml[];
    return enrichLookupFields(records, 'fiona_export_zu_kml');
  }
  static async getFionaExportZuKmlEntry(id: string): Promise<FionaExportZuKml | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FIONA_EXPORT_ZU_KML}/records/${id}`);
    const record = { record_id: data.id, ...data } as FionaExportZuKml;
    return enrichLookupFields([record], 'fiona_export_zu_kml')[0];
  }
  static async createFionaExportZuKmlEntry(fields: CreateFionaExportZuKml) {
    return callApi('POST', `/apps/${APP_IDS.FIONA_EXPORT_ZU_KML}/records`, { fields: cleanFieldsForApi(fields as any, 'fiona_export_zu_kml') });
  }
  static async updateFionaExportZuKmlEntry(id: string, fields: Partial<CreateFionaExportZuKml>) {
    return callApi('PATCH', `/apps/${APP_IDS.FIONA_EXPORT_ZU_KML}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fiona_export_zu_kml') });
  }
  static async deleteFionaExportZuKmlEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FIONA_EXPORT_ZU_KML}/records/${id}`);
  }
