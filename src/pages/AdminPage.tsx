...
import { FionaExportZuKmlViewDialog } from '@/components/dialogs/FionaExportZuKmlViewDialog';
...
const FIONAEXPORTZUKML_FIELDS = [
  { key: 'zip_datei', label: 'Fiona Export ZIP-Datei', type: 'file' },
  { key: 'kml_name', label: 'Name der KML-Ausgabedatei', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'koordinatensystem', label: 'Koordinatensystem der Quelldaten', type: 'lookup/select', options: [{ key: 'wgs84', label: 'WGS 84 (EPSG:4326)' }, { key: 'lv95', label: 'CH1903+ / LV95 (EPSG:2056)' }, { key: 'lv03', label: 'CH1903 / LV03 (EPSG:21781)' }, { key: 'utm32n', label: 'UTM Zone 32N (EPSG:32632)' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'koordinatensystem_sonstiges', label: 'Koordinatensystem (falls Sonstiges)', type: 'string/text' },
  { key: 'ebenen', label: 'Zu exportierende Ebenen (kommagetrennt)', type: 'string/text' },
  { key: 'geometrietyp', label: 'Geometrietypen einschliessen', type: 'multiplelookup/checkbox', options: [{ key: 'punkte', label: 'Punkte' }, { key: 'linien', label: 'Linien' }, { key: 'flaechen', label: 'Flaechen' }] },
  { key: 'attribute_exportieren', label: 'Attribute in KML exportieren', type: 'bool' },
  { key: 'anmerkungen', label: 'Anmerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'fiona_export_zu_kml', label: 'Fiona Export zu KML', pascal: 'FionaExportZuKml' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('fiona_export_zu_kml');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'fiona_export_zu_kml': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fiona_export_zu_kml': return (data as any).fionaExportZuKml as FionaExportZuKml[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fiona_export_zu_kml': return FIONAEXPORTZUKML_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fiona_export_zu_kml': return {
        create: (fields: any) => LivingAppsService.createFionaExportZuKmlEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFionaExportZuKmlEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFionaExportZuKmlEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'fiona_export_zu_kml' || dialogState?.entity === 'fiona_export_zu_kml') && (
        <FionaExportZuKmlDialog
          open={createEntity === 'fiona_export_zu_kml' || dialogState?.entity === 'fiona_export_zu_kml'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'fiona_export_zu_kml' ? handleUpdate : (fields: any) => handleCreate('fiona_export_zu_kml', fields)}
          defaultValues={dialogState?.entity === 'fiona_export_zu_kml' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['FionaExportZuKml']}
          enablePhotoLocation={AI_PHOTO_LOCATION['FionaExportZuKml']}
        />
      )}
      {viewState?.entity === 'fiona_export_zu_kml' && (
        <FionaExportZuKmlViewDialog
          open={viewState?.entity === 'fiona_export_zu_kml'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'fiona_export_zu_kml', record: r }); }}
        />
      )}
